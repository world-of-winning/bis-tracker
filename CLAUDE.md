...
- **`BIS` comes from Wowhead, `MYTHIC` from Maxroll.** One publisher for both was one account of the same fact, and a wrong row had nothing to contradict it — Maxroll filed a tier helm in Vengeance Demon Hunter's Neck row and the tracker carried it for a season. See `docs/adr/0006-wowhead-primary-maxroll-second-witness.md`.
  ...
  - **Maxroll's raid guide is still fetched, and read only as a second witness.** `scripts/cross-check.mjs` holds the rule, as a pure function over rows and facts. Two publishers naming different items for one slot is judgement and is **not** reported — the two lists do not even answer the same question. A **structural contradiction** is: a row whose slot label the item's own tooltip denies, or whose source names a place the client's loot table says the item does not drop. Those are reported, and they decide — where the sources disagree and only one row survives, the surviving row is taken.
  - The two faults are not the same problem. A **slot** fault says the row names the wrong item, and only another row fixes it: the contradicted row falls back to the other publisher, then to the value already in the data file, and failing both is kept and reported. A **source** fault says the row names the right item and mislabels where it drops — the label is rewritten from the loot table and the item kept, since swapping the item out over a wrong dungeon name would be the larger error.
  - **A slot the primary list never names** is the same harm as a contradicted row — Wowhead's Beast Mastery page lists no helm at all — so it is filled the same way: the second witness if its row stands up, then the data file.
  - A **third fault**: an item that drops only in instances this season does not run. Guides lag — Maxroll's Restoration Druid Mythic+ page named an ilvl-289 waist from Maisara Caverns, a dungeon Season 2 does not have. It falls back like a slot fault, and where nothing sound replaces it the row is **dropped** rather than kept. That is the one contradiction worse kept than missing: every other keeps a spec's slot count honest, this one sends a player to a retired dungeon.
  - A fallback the game contradicts too is not a fallback. The data file's own row has to pass the same check, or the run writes it back and the next run offers it again, and a bad row outlives every source of it.
  - `KNOWN_STATS` is pruned to the ids the file names, by whichever generator writes last (`find-alts`' `updateSpecFile`, which rewrites `ALTS` whole). An entry nothing references is an entry nobody rechecks.
  - The run never dies for one bad row, and every contradiction is listed again at the end of the run.
  - Sources naming a profession (`Crafting / Misc`, `Crafting Blacksmithing`, a bare `[skill=164]`) all normalize to `"Crafted"`. The app tests `source === "Crafted"` exactly to grade an item against the crafted ilvl ceiling, so the uniformity is load-bearing.
  - Their **stats are not** normalized alongside the source, and should not be. Most crafted rows carry `[]` because the secondaries are chosen at craft time and the tooltip states none; two items (`251513`, `239664`) carry fixed secondaries and say so. Neither is in the loot table, so both are genuinely crafted — the difference is the items, not a mislabelled row. Flattening them to one shape would be inventing a fact.
  - **`MYTHIC` is cross-checked where it overlaps.** Filtering Wowhead's list to dungeon sources gives six to nine rows — enough to cover that many of `MYTHIC`'s sixteen slots. The fault check itself needs no second witness, so a mislabelled row is caught on every slot either way.
  - `guide=NNNNN` source ids are not parsed. The source text is checked against the client's own loot table instead, which is a better witness than Wowhead's tag for the same fact.
- **`find-alts.mjs`** builds the season pool once — `buildSeasonPool()` joins the client's `JournalEncounterItem` through `JournalEncounter` to `JournalInstance` (via `scripts/wago-db2.mjs`) and keeps the instances named in `DUNGEONS` plus `CURRENT_RAIDS`. Non-gear drops (recipes, consumables, furnishings) have no inventory slot and fall out on their own. Then, per spec, every pool item the spec can *wear* becomes an alt: armour class, primary stat, class lock, weapon type and hand count are the gate. **Secondary stats are not a gate**, only the sort order. Nothing is preserved between runs — the pool is regenerated whole, which is what stops ALTS becoming append-only.
...
- **`CURRENT_RAIDS`** (in `shared.js`) is a **list**, maintained by hand beside `DUNGEONS`. A season runs more than one raid: this one runs The Venomous Abyss and The Tidebound Grotto, both handing over gear at the Myth cap of 334, and holding a single name kept a whole boss's loot out of every alt list while both publishers named six of its items as BiS. The loot table carries every raid ever shipped and marks no season — `DisplaySeasonID` is 0 on 23,902 of its 23,978 rows, and `JournalInstance` carries no season at all — so the judgement is a human's.
  - `buildSeasonPool` warns twice and decides never. First, the instance holding the highest item id should be one the pool admits; item ids say when an item was added, not which season runs it, and against a *list* that check is weak, since a second raid a few thousand ids below the first looks like any retired instance. Second — the check that would have caught this — **an instance the spec files name BiS items from, and the pool does not admit**, is a contradiction between two things already on disk, needing no threshold and no guess at what "recent" means.
  - Deciding what to leave out is part of the same judgement. Sporefall is the nearest thing excluded: its gear tops out at 298, it sits in Harandar rather than Atal'Utek, and no guide picks from it.
...
- **`--fix`**: Read-only cache, normalize data only, no network calls. Safe to run anytime. Rebuilding `KNOWN_STATS`, it will not write stats derived from an expired tooltip — those items keep what the file already holds, and the run says how many. "No network" is the contract; repeating a stale claim is not part of it. The source rewrite still reads an expired tooltip for its Tier marker, which is a set membership rather than a tuned value and does not drift the way stats do.
